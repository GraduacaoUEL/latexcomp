...
\hspace{2cm}Para criar uma nova seção, é necessário utilizar o comando section. Desta forma, o \LaTeX\ criará uma seção dita e, caso for requerido, será indexado automaticamente em um índice (comando \textbackslash index). É possivel fazer isto em 3 níveis: seção, sub-seção e sub-sub-seção.
...
\end{document}
\end{verbatim}
\hspace{2cm}É o encapsulamento principal; todas os outros ambientes, exceto ao pre-âmbulo, devem ser inseridos dentro deste. Caso contrário, o intepretador \LaTeX\ não dará a saída desejada se estiver fora do "ambiente".
...
\hspace{2cm}Cria listas em um ambiente. Existem 2 tipos de listas: \textit{enumerate}, \textit{itemize}. O último basta para listas simples, enquanto \textit{enumerate} cria listas enumeradas. Cada item deve ser colocado entre o ambiente após o comando \textbackslash \textit{item Nome do Item}. Estes podem ser detalhadas logo após utilizando o comando \textbackslash \textit{description Descrição}.
...
\hspace{2cm}Para escrever em itálico: (\textit{it}alic)
Sintaxe:
\begin{verbatim}
\textit{texto} 
\end{verbatim}
...
\hspace{2cm}Deixa o texto \textit{itálico}.
...
\hspace{2cm}Alinhamento à esquerda (flushleft), à direita (flushright) e centralizado (center).
